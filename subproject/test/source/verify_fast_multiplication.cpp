...
static bool verify_fast_multiplication_xz() {
    ...
    for (int e = impl::min_exponent + 1; e <= impl::max_exponent; ++e) {
        ...
        // Load cache
        auto const cache =
            jkj::dragonbox::policy::cache::full.get_cache<typename impl::format>(-minus_k);
...
int main() {
    ...
    std::cout
        << "[Verifying fast computation of xi and zi for the closer boundary case (binary32)...]\n";
    ...
    std::cout
        << "[Verifying fast computation of xi and zi for the closer boundary case (binary64)...]\n";
