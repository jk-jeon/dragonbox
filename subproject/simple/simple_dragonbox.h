...
namespace simple_dragonbox {
    ...
    struct float_format<float> {
        ...
        static compute_mul_parity_result compute_mul_parity(uint32_t two_f, uint64_t cache, int beta) {
            assert(beta >= 1);
            assert(beta <= 32);
            auto const r = umul96_lower64(two_f, cache);
            return {((r >> (64 - beta)) & 1) != 0, (UINT32_C(0xffffffff) & (r >> (32 - beta))) == 0};
        }
    ...
    struct float_format<double> {
        ...
        static compute_mul_parity_result compute_mul_parity(uint64_t two_f, uint128 cache, int beta) {
            assert(beta >= 1);
            assert(beta < 64);
            auto const r = umul192_lower128(two_f, cache);
            return {((r.high >> (64 - beta)) & 1) != 0,
                    (((r.high << beta) & UINT64_C(0xffffffffffffffff)) | (r.low >> (64 - beta))) == 0};
        }
    ...
    template <class Float, cache_type>
    struct cache_holder;

    template <>
    struct cache_holder<float, cache_type::full> {
        ...
        uint64_t get_cache(int k) const {
            ...
            if (offset == 0) {
                ...
                auto const pow5 = offset >= 7
                                      ? uint32_t(uint32_t(pow5_table[6]) * pow5_table[offset - 6])
                                      : uint32_t(pow5_table[offset]);
                auto mul_result = umul128(base_cache, pow5);
                auto const recovered_cache =
                    uint64_t((((mul_result.high << int(64 - alpha)) | (mul_result.low >> alpha)) + 1) &
    ...
    template <>
    struct cache_holder<double, cache_type::full> {
        ...
        uint128 get_cache(int k) const {
            ...
            if (offset == 0) {
                ...
                auto const high_to_middle =
                    uint64_t((recovered_cache.high << (64 - alpha)) & UINT64_C(0xffffffffffffffff));
                auto const middle_to_low =
                    uint64_t((recovered_cache.low << (64 - alpha)) & UINT64_C(0xffffffffffffffff));
    ...
    struct impl {
        ...
        static constexpr cache_holder<Float, CacheType> cache_;
        ...
        auto nearest(interval normal_interval, interval shorter_interval) {
            ...
            if (binary_exponent != 0) {
                ...
                if (two_fc == 0) {
                    ...
                    auto xi = format::compute_left_endpoint_for_shorter_interval_case(cache, beta);
                    auto zi = format::compute_right_endpoint_for_shorter_interval_case(cache, beta);
            ...
            // fine.
            auto const z_result = format::compute_mul(carrier_uint((two_fc | 1) << beta), cache);
            ...
            if (DecimalRoundMode == decimal_round_mode::dont_care) {
                ...
                if (!normal_interval.include_right_endpoint) {
                    ...
                    if (check_divisibility_and_divide_by_pow10<kappa>(r) && z_result.is_integer) {
        ...
        auto right_closed_directed() {
            ...
            do {
                if (r > deltai) {
                    ...
                    // Compare the fractional parts.
                    if (!format::compute_mul_parity(carrier_uint(two_fc - (shorter_interval ? 1 : 2)),
                                                    cache, beta)
                             .parity) {
                        break;
                    }
    ...
    namespace policy {
        namespace decimal_to_binary_rounding {
            using type = binary_round_mode;
            constexpr policy_<type, type::nearest_to_even> nearest_to_even;
            constexpr policy_<type, type::nearest_to_odd> nearest_to_odd;
            constexpr policy_<type, type::nearest_toward_plus_infinity> nearest_toward_plus_infinity;
            constexpr policy_<type, type::nearest_toward_minus_infinity> nearest_toward_minus_infinity;
            constexpr policy_<type, type::nearest_toward_zero> nearest_toward_zero;
            constexpr policy_<type, type::nearest_away_from_zero> nearest_away_from_zero;
            constexpr policy_<type, type::nearest_to_even_static_boundary>
                nearest_to_even_static_boundary;
            constexpr policy_<type, type::nearest_to_odd_static_boundary>
                nearest_to_odd_static_boundary;
            constexpr policy_<type, type::nearest_toward_plus_infinity_static_boundary>
                nearest_toward_plus_infinity_static_boundary;
            constexpr policy_<type, type::nearest_toward_minus_infinity_static_boundary>
                nearest_toward_minus_infinity_static_boundary;
            constexpr policy_<type, type::toward_plus_infinity> toward_plus_infinity;
            constexpr policy_<type, type::toward_minus_infinity> toward_minus_infinity;
            constexpr policy_<type, type::toward_zero> toward_zero;
            constexpr policy_<type, type::away_from_zero> away_from_zero;
        }
        namespace binary_to_decimal_rounding {
            using type = decimal_round_mode;
            constexpr policy_<type, type::to_even> to_even;
            constexpr policy_<type, type::to_odd> to_odd;
            constexpr policy_<type, type::away_from_zero> away_from_zero;
            constexpr policy_<type, type::toward_zero> toward_zero;
            constexpr policy_<type, type::dont_care> do_not_care;
        }
        namespace cache {
            using type = cache_type;
            constexpr policy_<type, type::full> full;
            constexpr policy_<type, type::compact> compact;
        }
    }
