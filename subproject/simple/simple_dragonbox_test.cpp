...
static bool test_simple_dragonbox(bool& success, Arg... arg) {
    ...
    for (std::size_t test_idx = 0; test_idx < test_count; ++test_idx) {
        ...
        jkj::simple_dragonbox::to_chars(x, buffer1, arg...);
    ...
    using format = jkj::simple_dragonbox::detail::float_format<Float>;
    ...
    for (int e = format::min_exponent; e <= format::max_exponent; ++e) {
        ...
        carrier_uint br = carrier_uint(e - format::exponent_bias) << format::significand_bits;
...
int main() {
    ...
    test_simple_dragonbox<float>(success, jkj::simple_dragonbox::policy::cache::compact);
    test_simple_dragonbox<double>(success);
    test_simple_dragonbox<double>(success, jkj::simple_dragonbox::policy::cache::compact);
    if (!success)
        return -1;
}
